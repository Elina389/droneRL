...
class SwarmCoverageEnv(ParallelEnv):
    ...
    def latlon_to_cell(self, lat, lon):
        """Inverse of cell_to_latlon: map a real (lat, lon) back to the grid
        cell (row, col) containing it. Returns None if the point falls
        outside the environment's bounding box. Used to turn a click on the
        map into a grid cell (e.g. for user-inflicted damage). Requires the
        env to have been built with a bbox."""
        if self.bbox is None:
            raise ValueError(
                "latlon_to_cell() requires the env to have been built with a bbox="
            )
        ...
        # row 0 is the northern edge (matches cell_bounds), so measure from north
