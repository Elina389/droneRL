...
def demo_rollout(n_drones=3, grid_size=10, n_obstacles=10, max_steps=50, seed=0, bbox=None,
    ...
    strategy : "random", "potential_field", or "bfs"
        ...
        "ppo" -- the LEARNED policy: a neural network trained with PPO
        reinforcement learning (see rl_train.py). No hand-coded rules; every
        move comes from a forward pass of the trained network on each drone's
        local observation. Requires a trained model at models/ppo_swarm.pt
        (run rl_train.py first). Trained with obs_window=3, so the env's
        obs_window must match.
    """
    ...
    # Policies are created once outside the loop. The stateful ones keep
    # per-drone memory across steps (previous cell for potential fields,
    # current target for BFS); the PPO policy holds the loaded network.
    ...
    ppo_policy = None
    if strategy == "ppo":
        from rl_policy import PPOPolicy  # lazy import: only pull in torch if used
    ...
    while env.agents:
        ...
        if strategy == "potential_field":
            ...
            actions = ppo_policy.actions(env)
