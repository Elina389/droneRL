...
class OccupancyGrid:
    ...
    def sense_from(self, pos, sensor_radius, true_obstacles):
        ...
        r0, c0 = pos
        changed = []
        visible_occupied = set()  # obstacle cells in line of sight THIS sweep
        R = sensor_radius
        ...
        # `changed` fires only on first sight of a cell; `visible_occupied`
        # fires every sweep a structure is in view, which is what lets a
        # drone RE-observe an already-known building and notice damage that
        # appeared after it was first seen (see server damage re-detection).
        return changed, visible_occupied
...
class UnknownTerrainPolicy:
    ...
    def __init__(self, sensor_radius=3):
        ...
        self.occ = None
        self._target = {}
        self.last_visible_occupied = set()  # structures in view this tick
    ...
    def reset(self, env):
        ...
        self._target = {}
        self.last_visible_occupied = set()
        # initial sweep from every drone's start cell
        for agent in env.agents:
            _, vis = self.occ.sense_from(env.positions[agent], self.sensor_radius, env.obstacles)
            self.last_visible_occupied |= vis
    ...
    def actions(self, env, priority=None, priority_weight=6.0):
        ...
        # 1. sense from every drone's current position (shared grid), and
        # collect every structure currently in view (for damage re-detection)
        self.last_visible_occupied = set()
        for agent in env.agents:
            _, vis = self.occ.sense_from(env.positions[agent], self.sensor_radius, env.obstacles)
            self.last_visible_occupied |= vis
