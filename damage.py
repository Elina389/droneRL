...
class DamageModel:
    ...
    def inflict(self, cell, severity=3):
        """Set (raise) the ground-truth severity of a structure cell -- used
        by the interactive 'inflict damage' tool so a user can damage a
        building and watch the swarm discover it. Only ever raises severity,
        never lowers it. Note this changes the ground truth only; a drone
        still has to SENSE the cell before it 'knows' about the new damage
        (that recognition happens in the server's re-detection loop)."""
        cell = tuple(cell)
        self.severity[cell] = max(self.severity.get(cell, 0), int(severity))
